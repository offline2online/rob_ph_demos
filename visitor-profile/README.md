...
merged to `main` on its own schedule, not tied to
`display-types-dsp-integration/`'s release cadence. See root `CLAUDE.md` →
"Live Visitor Profile and Display Types & DSP Integration" for the full
split rationale.
...
maintained boundary with `display-types-dsp-integration/`.
