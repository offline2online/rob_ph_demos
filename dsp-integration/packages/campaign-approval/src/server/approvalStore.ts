...
export function approvalStore(db: SqlDb) {
  ...
  const cache = new Map<string, ReturnType<SqlDb['prepare']>>()
  ...
  return {
    ...
    audit(campaignId: string, assetVersion: string, action: AuditAction, actor: string | null, reason: string | null, at: string, assetReasons?: AssetRejection[]) {
      stmt('INSERT INTO campaign_approval_audit (id, campaign_id, asset_version, action, actor, reason, asset_reasons, at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    ...
    isHumanCleared(campaignId: string, assetId: string, contentHash: string): boolean {
      const r = stmt('SELECT content_hash FROM campaign_approval_asset_clearance WHERE campaign_id = ? AND asset_id = ?').get(campaignId, assetId) as { content_hash: string } | undefined
