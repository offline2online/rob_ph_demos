...
import { STATUSES, type ApprovalStatus } from '../types'
...
export const approvalRoutes = (service: ApprovalService, hooks: ApprovalRouteHooks): FastifyPluginAsync => async (app) => {
  ...
  app.post<{ Params: { id: string }; Body: { assetVersion?: string; reason?: string } }>('/campaigns/:id/reject', async (req, reply) => {
    hooks.guard(req)
    hooks.requireApprover(req)
    ...
    try { return await service.reject(req.params.id, req.body?.assetVersion ?? '', hooks.reviewer(req), req.body.reason) } catch (e) { return send(reply, e) }
  })
}
