  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-start gap-3.5">
        <Field label={<span className="whitespace-nowrap">Currency</span>} htmlFor="currency" tip="Used for the floor CPM, every effective floor and billing. Bid requests carry it as the bid floor currency." className="w-44">
        ...
        <Field label={<span className="whitespace-nowrap">Floor price (CPM)</span>} htmlFor="floorCpm" tip={FLOOR_TIP} tipWidth={400} className="w-44">{num('floorCpm', 1, '100')}</Field>
        <Field label={<span className="whitespace-nowrap">Personalised multiplier</span>} htmlFor="personalisedMultiplier" tip={PERSONALISED_TIP} tipWidth={400} className="w-56">{num('personalisedMultiplier', 0.05, '1.5')}</Field>
        <Field label={<span className="whitespace-nowrap">Interactive cost per engagement</span>} htmlFor="interactiveCpe" tip={INTERACTIVE_TIP} tipWidth={400} className="w-72">{num('interactiveCpe', 0.05, '0.50', { precision: 2, prefix: s.currency })}</Field>
      </div>
      ...
      <div className="flex flex-wrap items-start gap-3.5">
        <Field label={<span className="whitespace-nowrap">Auction opens</span>} htmlFor="auctionOpensHours" tip="How long before the auction cutoff bidding for a play window opens, for example 7 days." className="w-64">
          <DaysHours id="auctionOpensHours" hours={s.auctionOpensHours} onChange={(v) => set('auctionOpensHours', v)} />
        </Field>
        <Field label={<span className="whitespace-nowrap">Play-window length</span>} htmlFor="playWindowHours" tip="The minimum period a won slot is held, in days and hours, for example 24 hours or 7 days. It can't change while future windows are bid on or booked." className="w-64">
          <DaysHours id="playWindowHours" hours={s.playWindowHours} onChange={(v) => set('playWindowHours', v)} />
        </Field>
        <Field label={<span className="whitespace-nowrap">Auction cutoff time</span>} htmlFor="auctionCutoffTime" tip="The daily time by which bids must be in. The auction for the next play window runs then; 18:00 gives six hours before a midnight window." className="w-48">
