...
/* The funnel: a popup of every value in the column, with Clear Filter. */
function SetFilter(props: Params) {
  ...
  const menu = (
    <div className="rounded-md bg-white p-2 shadow-lg" style={{ border: `1px solid ${T.borderSubtle}`, minWidth: 200 }}>
      {values.length > 8 && (
        <Input size="small" className="mb-2" placeholder="Search..." aria-label={`${props.label} filter search`} value={search} onChange={(e) => setSearch(e.target.value)} />
      )}
      <div className="max-h-[260px] overflow-auto">
        {shown.map((v) => (
          <div key={v} className="px-1 py-1">
            <Checkbox checked={chosen.includes(v)} onChange={(e) => apply(e.target.checked ? [...chosen, v] : chosen.filter((x) => x !== v))}>
