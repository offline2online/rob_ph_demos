  ...
  return (
    <div className="min-h-screen w-full bg-white p-5" style={{ color: T.text, fontSize: 14 }}>
      ...
      <div className="flex items-start">
        {/* A route can opt out (RouteHandle.hideNav) when it stands alone in its
            own tab — no Display Types / DSP Integration nav to show beside it. */}
        {nav.length > 0 && (
          <nav
        ...
        <div className={`min-w-0 flex-1 ${nav.length > 0 ? 'pl-5' : ''}`}>{children}</div>
      </div>
    </div>
  )
}
