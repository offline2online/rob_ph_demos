...
describe('Exchange settings and sellers.json (spec §7)', () => {
  it('returns the seller of record and where sellers.json is published', async () => {
    const res = await buildApp(testContext()).inject({ method: 'GET', url: '/api/admin/v1/exchange' })
  ...
  it('publishes sellers.json with the platform defaults (PUBLISHER, not confidential)', async () => {
    const res = await buildApp(testContext()).inject({ method: 'GET', url: '/sellers.json' })
  ...
  it('sellers.json is 404 until complete, and with the flag off', async () => {
    const empty = buildApp(testContext({ seeded: false }))
    ...
    expect((await buildApp(testContext({ flag: false })).inject({ method: 'GET', url: '/sellers.json' })).statusCode).toBe(404)
  })

  it('saves all four fields and republishes', async () => {
    const app = buildApp(testContext({ seeded: false }))
  ...
  it.each([['GET'], ['PUT']] as const)('%s /admin/v1/exchange returns 404 with the flag off', async (method) => {
    const res = await buildApp(testContext({ flag: false })).inject({ method, url: '/api/admin/v1/exchange', payload: valid })
