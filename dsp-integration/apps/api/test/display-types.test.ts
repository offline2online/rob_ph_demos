...
const setup = (opts: Parameters<typeof testContext>[0] = {}) => {
...
describe('read side used by the slot picker (flag-gated)', () => {
  ...
  it('GET /admin/v1/advertisers: admin only, with effective floors', async () => {
    const res = await setup().app.inject({ method: 'GET', url: '/api/admin/v1/advertisers' })
    ...
    const forbidden = await setup({ role: 'hq_user' }).app.inject({ method: 'GET', url: '/api/admin/v1/advertisers' })
  ...
  it.each(['/partners', '/advertiser-settings', '/advertisers'])('%s returns 404 with the flag off', async (path) => {
    const res = await setup({ flag: false }).app.inject({ method: 'GET', url: `/api/admin/v1${path}` })
    expect(res.statusCode).toBe(404)
  })
})
