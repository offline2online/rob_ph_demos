...
export const sqlitePlaybackSource = (db: Db): PlaybackSource => ({
  listPlays({ campaignId, from, to }) {
    const rows = (campaignId
      ? db.prepare('SELECT * FROM plays WHERE campaign_id = ? AND played_at >= ? AND played_at < ? ORDER BY played_at').all(campaignId, from, to)
      : db.prepare('SELECT * FROM plays WHERE played_at >= ? AND played_at < ? ORDER BY played_at').all(from, to)) as unknown as Row[]
