...
| Auth and roles | Stand-in session from `POC_ROLE` (`hq_admin`, `hq_marketing` or `hq_helpdesk`); partner bearer tokens from config |
...
| `Slot.supportedTargeting`, `AvailableInventoryRow.supportedTargeting`, `Position.supportedTargeting`, `PUT /admin/v1/available-inventory`, and the error code `targeting_not_supported` (20 Sep, Rob) | A slot has to say which kinds of campaign it will take — localised, personalised or interactive — set on Available Inventory, published to DSPs on the position, and enforced on a bid or reservation. Localised only for a slot nobody has changed |
...
Q1–Q15 below are all resolved or accepted. What is still open, from the
week's work (21 Sep):

- **Billing an engagement.** Interactive is now priced per engagement
  (`interactiveCpe`, published on a position as `costPerEngagement`), but
  nothing bills it: the stand-in playback data counts plays, not QR scans,
  so billing still charges CPM × realised VAC-d only. Wiring the fee up
  needs an engagement count from the platform — a `PlaybackSource`-style
  stand-in would do for the POC. **Nobody has asked for it yet; it should be
  a decision, not an oversight.**
- **Who may activate a campaign.** Approving and rejecting are admin-only
  (`canApprove`), but the activation toggle on Campaign Status is not: a
...
since the picker was removed (Rob, 20 Sep) and nothing edits it now.
...
15. ~~Roles in the POC~~ **Resolved (Rob, 20 Sep):** the inventory moved to
    Advertisers / Inventory *because* marketing users need it, so that
    screen is no longer admin-only. The POC now carries all three roles and
    enforces them; a marketing user reads the per-advertiser settings but
...
| Campaign brief captured with the booking (Rob, 20 Sep) | Built | `POST /v1/campaigns` takes an optional `brief` in the shape of the platform's own Campaign Brief tab (checked on demo.personalisationhub.com): the advertiser's own `details`, `landingPageUrl`, `promotedProducts`, `skus`, `targetAudiences`, `objective` and `touchPoints` (Digital Signage for now; Kiosk accepted for later). Every field is optional, each is length-limited, unknown fields are rejected, and nothing in it targets or prices anything — it is what the reviewer reads. Stored beside the campaign (migration 0016) and returned on `GET /admin/v1/campaigns`. The seeded advertiser campaigns now carry briefs. Contract: `CampaignBrief` on `CampaignCreate` and `Campaign`. Tests: API +2 |
| Column filters match the platform (Rob, 20 Sep) | Built | Checked against HQ Admin → Displays & Devices → Assigned Devices and rebuilt to match, in `shared/TableFilters.tsx`: a column gets **one** filter, never two, and the label row never shows a funnel. **Search columns** are a plain input that filters as you type, with the round ✕ inside it as the only way to clear. **Set columns** are a funnel alone — grey outline when idle, solid primary when active — opening a popup with a Search box for long lists, a checkbox per value (multi-select, OR) and a primary **Clear Filter** button, disabled while nothing is ticked. The count line above the table switches to **"Showing X of N …"** while filtered. Campaign Status uses them (Status, Advertiser and DSP as set filters; Name as search). The booking schedule's DSP and advertiser controls are **column filters too** (Rob: "column filter, like everywhere else"): pinned **DSP** and **Advertiser** columns carry the same funnel and popup, but the choice lives in the URL and the server applies it, so it narrows every window rather than only the rows on screen (`externalSetColumn`, single-select). No table in this build has a page-level filter control any more. The **ph-designer skill** now spells all of this out in `references/components.md` §3.1, including that filter state belongs in the URL, so it is the rule from here on |
| Campaign Status: schedule column, value filters and a row menu (Rob, 20 Sep; three tickets) | Built | A **Schedule** first column shows the next play window the advertiser booked and how many it holds, sorted earliest first so what is up next is at the top (new `schedule` on the admin `Campaign`: `nextWindowStart`, `bookedWindows`, derived from the windows it holds). The Status, Advertiser and DSP filters are now **selects that list the values in the table** and name themselves (`shared/SelectFilter.tsx`, driving AG Grid's own filter), as the design system's filter row asks. A **⋯ row menu** (pinned right, as the skill's tables do) opens the campaign, approves, rejects with a reason, or activates and deactivates, each disabled when it doesn't apply. Opening Campaign Status with `?advertiserId=` filters it to one advertiser, with a chip to clear |
| Advertisers / Inventory: campaign counts open the filtered list, Open lands on the slot (Rob, 20 Sep; two tickets) | Built | The campaign-count icons are now a button that opens **Campaign Status filtered to that advertiser**. Available Inventory's **Open** goes to the display type with **Playlist Settings already expanded** (`?panel=playlist`), which is where slot assignment lives |
| Booking schedule: DSP first, then its advertisers (Rob, 20 Sep) | Built | The DSP select comes first and the advertiser select to its right lists only that DSP's advertisers; changing the DSP drops an advertiser it doesn't bring. The endpoint returns the DSPs and their advertisers (`dsps` on the booking schedule), so the page needs no admin-only data — a marketing user gets the same filters |
| Available Inventory moves to Advertisers / Inventory (Rob, 20 Sep) | Built | The inventory table (display type, playlist, slot, position with its DSP, and the Open link) and the Booking schedule link left **DSP Integration → Advertiser settings** and now sit under the advertisers table. The nav item and page are **Advertisers / Inventory**; Advertiser settings' tooltip points there. Still admin only, as that screen already was |
| Advertisers / Inventory: filters on both tables, and what targeting each slot supports (Rob, 20 Sep) | Built | Both tables on the page now filter by column, the platform's way (`shared/TableFilters.tsx`): **Advertisers** has search on the name and funnels on Via and Campaign approval; **Available Inventory** has search on Display type and Position and funnels on Playlist, Slot and Targeting supported, each with its own "Showing X of N" line. Playlist Management was left alone, as asked. The Display type column is narrower, making room for a new **Targeting supported** column: three ticks — **Localised**, **Personalised**, **Interactive** — per slot, **localised only** by default, the last one ticked not removable, saved by the page's own Save changes (`PUT /admin/v1/available-inventory`, admin only; a marketing user sees the ticks disabled). It is not authoring-only: it rides on the slot (`Slot.supportedTargeting`), is published to DSPs on the position, and a bid or reservation for a campaign of an unsupported type is refused `targeting_not_supported` — in the API, in the auction's re-check of API bids, and on a DSP's OpenRTB bid. Tests: API +3, admin +2 |
| Assigned to moves off the display type, and both inventory fields become pill multi-selects (Rob, 20 Sep) | Built | **Display Types → Slot assignment** is now two editable columns, Label and Owner (Headquarters / Advertiser / Stores); the Assigned-to column is gone, and with it the store-scope select (a Stores slot takes *Store staff*, Rob: drop it). The slot card still shows what the position is assigned to, read-only. The API keeps the assignment and supported targeting across a save of the slots, and drops both when a slot stops being an Advertiser slot. **Available Inventory** gains **Assigned to**: one multi-select listing every DSP, every advertiser (labelled with its DSP) and *Whitelist only*, each choice a pill in the cell, nothing chosen showing *All DSPs*. Advertisers and Whitelist only are mutually exclusive; an advertiser's DSP is added on save. **Targeting supported** is now the same control (pills, not tick boxes). Underneath, a slot carries `partnerIds[]` and `advertisers[]` instead of one of each — so a position can be sold through several DSPs or held for several advertisers — and every buy path follows: visibility in the Inventory API, the DSPs an auction asks, the reservation route and the auction's re-checks. A slot saved in the old shape still reads correctly (`assignedOf`). Validation moved with the field: unknown DSP, an advertiser that is not a seat on the chosen DSPs, both advertisers and whitelist, an empty whitelist, and a blacklisted advertiser (one already held stays). Tests: API +2 and 4 rewritten, admin +1 rewritten |
| Interactive is a cost per engagement, not a multiplier (Rob, 21 Sep) | Built | **Interactive multiplier → Interactive cost per engagement** (`interactiveCpe`, migration 0017, default 0.50, entered to the cent with the currency prefix). The point of an interactive campaign is the scan, so it is priced per engagement: the campaign clears the ordinary CPM floor (or the personalised one) for its plays and pays the fee for each QR Control scan on top, unscaled by the advertiser's floor multiplier. `effectiveFloorCpm` is down to `localised` and `personalised`, and a position now publishes `costPerEngagement` to DSPs. **Engagements are not billed yet** — the stand-in playback data counts plays, not scans — so this prices and publishes the fee without billing it; wiring that up needs an engagement count from the platform. Tests: API 2 rewritten, +1 |
| Pricing tooltips say their own thing (Rob, 21 Sep) | Built | The personalised and interactive tooltips no longer repeat the floor price's VAC-d worked example: each explains its own field and relates itself to the floor ("It multiplies the floor price…", "Charged per engagement, on top of the CPM…"). The floor tooltip keeps the table. Tests: admin 1 rewritten |
| Interactive targeting needs QR Control (Rob, 21 Sep) | Built | Available Inventory flags the display types with **QR Control** enabled (a QR glyph beside the name, with a tooltip), because that is what a visitor engages with. On a slot whose display type has it off, **Interactive** is greyed out in the Targeting supported picker and carries the reason in place — *QR Control required to support an interactive engagement* — and `PUT /admin/v1/available-inventory` refuses it with the same words. Tests: API +1, admin +1 |
| Booking schedule: only advertisers that have bookings (Rob, 21 Sep) | Built | The advertiser filter lists only advertisers with something booked **in the range on screen** (computed before any filter is applied), and choosing one leaves only the positions it actually holds, instead of every position with its windows blanked. On the advertisers table, **Bookings** is only offered to an advertiser with bookings from the current window on (new `Advertiser.bookings` count) — there is nothing to open otherwise. Tests: API +1, admin +1 |
| Booking schedule leads with the schedule (Rob, 21 Sep) | Built | The page opens on the **Schedule** grid, with **Booking revenue** and **By campaign type** under it. The schedule is what the page is for; the money reads as its summary rather than standing in front of it |
| A hosted prototype to iframe into HQ Admin (Rob, 21 Sep) | Built | The admin UI built with its API captured into a snapshot (`scripts/capture-demo.mjs` → `src/demo/staticApi.ts`), published at `/dsp-integration/prototype/`. Reads come from the snapshot, the clock is pinned to the capture so the schedule's range matches it, creatives are copied beside it, and routes move to the hash so a deep link and a refresh work inside an iframe. **Writes are refused with a message**, rather than appearing to save. The old artifact prototype in `display-types-dsp-integration/` — and that folder, the renamed `experience-templates/` — was removed at the same time (Rob), with its history and its deploy train's 8 unmerged commits kept on the tag `archive/display-types-dsp-integration` |
| The board's copy of the docs is synced by script, not by hand (Rob, 21 Sep) | Built | `npm run board:sync` (`scripts/sync-board-docs.mjs`) reads `REQUIREMENTS.md` and `README.md` off disk, PATCHes the project's `requirementsMd` / `readmeMd` on the board, reads them back and fails unless they match byte for byte; `-- --check` reports drift without writing. It needs `BOARD_API_KEY` in `.env` to write; **`-- --check-mcp <file>` needs nothing at all** — hand it the result of the board MCP connector's `get_project_docs` and it compares locally, names the sections that moved and exits 1 when the board is behind, so an agent that can read the board but not write to it can still report the drift. The rule now sits in the root `CLAUDE.md` and in the Routine agent's instructions: the repo file is the source of truth and the board follows it **in the same session the file changes**, and a long specification is never retyped through a model to get there. The README was synced through the board's MCP tools today; the requirements are waiting on the key |
| Sample bookings for every brand (Rob, 21 Sep) | Built | `seed/bookings.ts`: for each advertiser a **connected** DSP brings, an approved and activated campaign (with creative) and three booked play windows — the first reserved at an agreed price, the rest won at auction, at CPMs between 120 and 195. It books whatever advertiser positions the database has, walks each advertiser forward so two sharing a position never want the same window, scores a position for VAC-d if nothing has (412 viewers per display, as in API.md's example), and skips anything already written or already sold, so it is safe to re-run. A fresh database gets them from `seed()`; an existing one takes them from `npm run db:bookings`. Tests seed a clean forward schedule unless they ask (`testContext({ bookings: true })`). Tests: API +1 |
| Who sees each section (Rob, 20 Sep) | Built | New *Who sees each section* subsection in `REQUIREMENTS.md` (mirrored into the board's Requirements doc), and the POC now has the three roles: `POC_ROLE` = `hq_admin`, `hq_marketing` or `hq_helpdesk`, with scopes `admin` / `approver` / `sections`. **DSP Integration is admin only** (exchange, advertiser settings, shared variables and the DSP pages all 403 for anyone else). **Display Types, Playlist Management, Advertisers / Inventory and Campaign Status** are admin *and* marketing; on Advertisers / Inventory a marketing user reads the advertisers and inventory but the approval toggle and floor multiplier are disabled and the save bar is hidden (Rob, 20 Sep — pricing and approval policy stay with an admin; `PUT /admin/v1/advertisers` is admin only). **Help desk users get nothing**: no nav, a short "ask an administrator" line, and every admin endpoint except the session itself returns 403. Contract: the session role enum is now the three roles, with the scopes described. Tests: API +2, admin +2. Browser-checked as all three roles |
| Advertisers: campaigns by status, and a way into their bookings (Rob, 20 Sep; two tickets) | Built | A **Campaigns** column shows each advertiser's campaigns by approval status as icons with counts (approved, awaiting approval, rejected, draft; each with a tooltip), from a new `campaigns` object on `GET /admin/v1/advertisers`. A **Bookings** action opens the booking schedule in a new tab, filtered to that advertiser. The schedule itself gained **Advertiser** and **DSP** filters (`advertiserId` / `partnerId` on the endpoint, kept in the URL). The approval module's service gained `statusOf(campaignId)` so a host screen can ask for the word without loading the whole review |
| Booking schedule: its own page, campaign types, daily / weekly / monthly (Rob, 20 Sep) | Built | It moved out of the DSP Integration section to its own route (`/booking-schedule`) and opens in a **new tab** from Available Inventory or an advertiser. Each booking now shows its **campaign type** (baseline, localised, personalised, interactive), and a **By campaign type** summary says what is selling — from `byPricingType` on the endpoint. A Daily / Weekly / Monthly switch regroups the columns: daily is one play window per column; weekly and monthly count how much of each period is sold ("3 of 7 booked") with the advertisers and revenue in the tooltip. The range follows the view (14 windows, 12 weeks, 3 months — the endpoint allows 92 days at a time). Tests: API +2, admin +0 (updated). Browser-checked in all three views |
