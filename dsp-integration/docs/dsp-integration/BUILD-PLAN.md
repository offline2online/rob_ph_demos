...
15. **Roles in the POC (open).** Rob's note (now in the spec) gives three
    kinds of user: admin, marketing and help desk. The POC's stand-in
    session has two roles, so only *Advertisers / Inventory* is admin-only
    today and DSP Integration is still shown to `hq_user`. Should the POC
    grow a third stand-in role and gate DSP Integration on admin, or is the
    spec note enough until the platform's own roles arrive?
...
| Who sees each section (Rob, 20 Sep) | Noted in the spec | New *Who sees each section* subsection in `REQUIREMENTS.md` (and the board's Requirements doc): **DSP Integration is admin only**; **Display Types, Playlist Management, Advertisers / Inventory and Campaign Status** are also shown to **marketing users**; **help desk users see none of it**, enforced server-side as well as in the navigation. Not built: the POC's stand-in session only has `hq_admin` and `hq_user`, so today it distinguishes admin from non-admin (Advertisers / Inventory is admin only; DSP Integration is not yet restricted). Q15 |
