...
| The board's copy of the docs is synced by script, not by hand (Rob, 21 Sep) | Built | `npm run board:sync` (`scripts/sync-board-docs.mjs`) reads `REQUIREMENTS.md` and `README.md` off disk, PATCHes the project's `requirementsMd` / `readmeMd` on the board, reads them back and fails unless they match byte for byte; `-- --check` reports drift without writing. It needs `BOARD_API_KEY` in `.env`. The rule now sits in the root `CLAUDE.md` and in the Routine agent's instructions: the repo file is the source of truth and the board follows it **in the same session the file changes**, and a long specification is never retyped through a model to get there. The README was synced through the board's MCP tools today; the requirements are waiting on the key |
