...
import { INITIAL_TYPES, INITIAL_PLAYLISTS, INITIAL_PARTNERS, INITIAL_COMPANY_LISTS, INITIAL_EXCHANGE, INITIAL_DISPLAYS } from "./model/data.js";
import {
...
import AdvertisersView from "./views/AdvertisersView.jsx";
import { SaveBar, InfoTip } from "./ui.jsx";
...
/* ---------------------------- release scope -------------------------------
   The first release is Display Types, Playlist Management, DSP Integration
   and Advertisers. Experience Layout — templates, the surface layer — is built
...
/* Not in this release. */
const EXCLUDED_ELEMENTS = ["ctas", "qr_control"];
...
const Panel = ({ title, children, open, onToggle, badge, summary }) => (
  <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 8, marginTop: 16, overflow: "hidden" }}>
    <div onClick={onToggle} style={{ padding: "12px 16px", background: T.surfaceAlt, borderBottom: open ? `1px solid ${T.borderSubtle}` : "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontSize: 13, letterSpacing: "0.3px", flexWrap: "wrap" }}>
      <Icon name={open ? "expand_more" : "chevron_right"} size={18} style={{ color: T.muted }} />
      <span style={{ minWidth: 150 }}>{title}</span>
      <span style={{ flex: 1, display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", letterSpacing: 0 }}>{summary}</span>
      {badge}
    </div>
    {open && <div style={{ padding: 16 }}>{children}</div>}
  </div>
);
/* Confirmation modal. When `blocked` is set the destructive action is
   disabled and the reason is shown instead. */
const ConfirmModal = ({ title, children, blocked, confirmLabel = "Delete", onConfirm, onCancel }) => (
  <div onClick={onCancel} style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
    <div onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true"
      style={{ width: "100%", maxWidth: 460, background: "#fff", borderRadius: 8, boxShadow: "0 12px 32px rgba(0,0,0,0.18)", overflow: "hidden" }}>
      ...
      <div style={{ padding: "4px 20px 16px", fontSize: 13, lineHeight: 1.55, color: T.text }}>{children}</div>
      <div style={{ padding: "12px 20px", borderTop: `1px solid ${T.borderSubtle}`, display: "flex", justifyContent: "flex-end", gap: 8 }}>
        <Btn onClick={onCancel}>{blocked ? "Close" : "Cancel"}</Btn>
        <Btn variant="danger" disabled={!!blocked} onClick={onConfirm} title={blocked || undefined} style={{ background: T.error, color: "#fff", border: `1px solid ${T.error}` }}>{confirmLabel}</Btn>
      </div>
    </div>
  </div>
);

/* One item in a panel's collapsed summary. tone: on | off | default. */
const SumChip = ({ icon, children, tone = "on", color }) => {
  const c = tone === "on" ? (color || T.primary) : T.muted;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 4, height: 22, padding: "0 8px", borderRadius: 9999, fontSize: 11.5,
      border: `1px solid ${tone === "on" ? c : T.border}`, color: c, background: tone === "on" ? "#fff" : "rgba(0,0,0,0.03)", whiteSpace: "nowrap" }}>
      {icon && <Icon name={icon} size={13} />}{children}
    </span>
  );
};
const FEATURE_SHORT = { in_store_radio: "In-Store Radio", qr_control: "QR Control", proximity_mist: "MIST", ai_agent_playback: "AI Agent", vision_ai: "Vision/AI" };
...
export default function App() {
  ...
  const go = (k) => { if (k === nav) return; if (unsaved && !window.confirm("You have unsaved changes. Discard them?")) return; setUnsaved(false); setNav(k); };
  ...
  const NAV = [
    { key: "types", label: "Display Types", icon: "dashboard_customize" },
    ...
    { key: "partners", label: "DSP Integration", icon: "handshake" },
    { key: "advertisers", label: "Advertisers", icon: "sell" },
    ...
    <div style={{ minHeight: "100vh", background: "#fff", padding: 20, fontFamily: FONT, color: T.text, fontSize: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start" }}>
        <div style={{ width: navCollapsed ? 56 : 230, flexShrink: 0, borderRight: `1px solid ${T.borderSubtle}`, position: "sticky", top: 20, maxHeight: "calc(100vh - 40px)", overflowY: "auto", overflowX: "hidden", transition: "width .15s" }}>
          {NAV.map((n) => {
            const a = nav === n.key;
            return <div key={n.key} onClick={() => go(n.key)} title={navCollapsed ? n.label : undefined}
        ...
        <div style={{ flex: 1, minWidth: 0, paddingLeft: 20 }}>
          {nav === "types" && <TypesView displays={INITIAL_DISPLAYS} types={types} setTypes={setTypes} playlists={playlists} setPlaylists={setPlaylists} sel={selType} setSel={setSelType} templates={templates} partners={partners} companyLists={companyLists} goToPartners={() => { setSelPartner(COMPANY_LISTS); go("partners"); }} onDirtyChange={setUnsaved} />}
          ...
          {nav === "advertisers" && <AdvertisersView partners={partners} lists={companyLists} setLists={setCompanyLists} onDirtyChange={setUnsaved} />}
          {nav === "partners" && <PartnersView partners={partners} setPartners={setPartners} companyLists={companyLists} setCompanyLists={setCompanyLists} exchange={exchange} setExchange={setExchange} types={types} playlists={playlists} goToType={(id) => { setSelType(id); go("types"); }} sel={selPartner} setSel={setSelPartner} onDirtyChange={setUnsaved} />}
        </div>
      </div>
    </div>
  );
...
function TypesView({ types: savedTypes, setTypes: saveTypes, playlists: savedPlaylists, setPlaylists: savePlaylists, sel, setSel: selectRaw, templates, partners, companyLists, goToPartners, onDirtyChange, displays = [] }) {
  const [delType, setDelType] = useState(null);
  /* Same save model as DSP Integration: edits go to a draft and only reach
     the saved records on Save changes; Cancel restores them. */
  const [types, setTypes] = useState(savedTypes);
  const [playlists, setPlaylists] = useState(savedPlaylists);
  const dirty = JSON.stringify([types, playlists]) !== JSON.stringify([savedTypes, savedPlaylists]);
  useEffect(() => { if (onDirtyChange) onDirtyChange(dirty); }, [dirty]);
  const saveAll = () => { saveTypes(types); savePlaylists(playlists); };
  const cancelAll = () => { setTypes(savedTypes); setPlaylists(savedPlaylists); if (!savedTypes.some((t) => t.id === sel)) selectRaw(savedTypes[0]?.id); };
  const leaveOk = () => { if (!dirty) return true; if (!window.confirm("You have unsaved changes. Discard them?")) return false; setTypes(savedTypes); setPlaylists(savedPlaylists); return true; };
  const setSel = (v) => { if (v === sel || !leaveOk()) return; selectRaw(v); };
  const [open, setOpen] = useState({ playlist: false, web: false, phantom: false, features: false, zones: false });
  ...
  /* Collapsed-panel summaries: what is enabled, and where defaults changed. */
  const overrides = Object.keys(ps).filter((k) => k !== "maximumCampaignsPlayedInRotation" && ps[k] !== null && ps[k] !== undefined);
  const ownerCount = (o) => slots.filter((x) => x.owner === o).length;
  const playlistSummary = (
    <>
      {isCapped(d) && <SumChip icon="view_week">{slotCount(d)} slots</SumChip>}
      {isCapped(d) && ["internal", "advertiser", "retail"].filter((o) => ownerCount(o) > 0).map((o) => (
        <SumChip key={o} icon={SLOT_OWNERS[o].icon} color={SLOT_OWNERS[o].color}>{ownerCount(o)} {SLOT_OWNERS[o].label}</SumChip>
      ))}
      {overrides.length > 0 && <SumChip icon="tune">{overrides.length} setting{overrides.length > 1 ? "s" : ""} changed</SumChip>}
      {!isCapped(d) && overrides.length === 0 && <SumChip tone="default">Default settings</SumChip>}
    </>
  );
  const pa = d.qrControl.phantomArea;
  const phantomSummary = pa.enabled
    ? <><SumChip icon="crop_free">{pa.width}×{pa.height}</SumChip><SumChip icon="place" tone={pa.position ? "on" : "default"}>{pa.position || `Default (${PHANTOM_DEFAULT_POSITION})`}</SumChip></>
    : <SumChip tone="default">Not defined</SumChip>;
  const featuresOn = FEATURES.filter((f) => COMPANY_AVAILABILITY[f.key] && featCfg(d, f.key).on);
  const featuresSummary = featuresOn.length
    ? featuresOn.map((f) => <SumChip key={f.key} icon={f.icon} color={T.success}>{FEATURE_SHORT[f.key] || f.label}</SumChip>)
    : <SumChip tone="default">None enabled</SumChip>;
  const zonesSummary = d.multiZone.enabled
    ? <SumChip icon="grid_view">{zones.length} zone{zones.length !== 1 ? "s" : ""}</SumChip>
    : <SumChip tone="default">Single zone</SumChip>;
            ...
            <select value={d.element?.type}
              ...
              {WEB_ELEMENT_GROUPS.map((g) => (
                <optgroup key={g} label={g}>
                  {WEB_ELEMENTS.filter((x) => x.group === g && !EXCLUDED_ELEMENTS.includes(x.key)).map((x) => <option key={x.key} value={x.key}>{x.name}</option>)}
                </optgroup>
              ))}
            </select>
  ...
  return (
    <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
      <div style={{ width: 260, flexShrink: 0, position: "sticky", top: 20, maxHeight: "calc(100vh - 40px)", overflowY: "auto", overflowX: "hidden" }}>
          ...
          {types.map((t) => {
            ...
            return <div key={t.id} onClick={() => setSel(t.id)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 7, minWidth: 0 }}>
                ...
                <span style={{ flex: 1, minWidth: 0, fontSize: 13, color: a ? T.primary : T.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</span>
                <span role="button" title="Delete display type" onClick={(e) => { e.stopPropagation(); setDelType(t); }}
                  style={{ display: "inline-flex", padding: 3, borderRadius: 4, cursor: "pointer", color: T.micro, flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* ---- PLAYLIST SETTINGS ---- */}
        {caps(d).campaigns && (
        <Panel title="PLAYLIST SETTINGS" open={open.playlist} onToggle={() => toggle("playlist")} summary={playlistSummary}>
          <Row>
          ...
          {isCapped(d) && (
            <div style={{ marginBottom: 16 }}>
              <Label><span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>Slot assignment<InfoTip size={14} text="Headquarters: filled from eligible HQ campaigns by priority. Advertiser: sold through a DSP, open to RTB bidding by default, or reserved to one of that DSP's advertisers. Stores: store staff activate approved campaigns. Ownership decides who can fill the slot; playback itself is unchanged." /></span></Label>
        ...
        {!isWebTP(d.touchPoint) && (
        <Panel title="PHANTOM ZONE" open={open.phantom} onToggle={() => toggle("phantom")} summary={phantomSummary}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <span style={{ fontSize: 13.5, display: "inline-flex", alignItems: "center", gap: 5 }}>Define phantom zone<InfoTip size={14} text="The phantom zone sits outside campaign rotation, so anything placed in it survives every campaign transition. Defining it makes Enable QR Control available." /></span>
        ...
        {!isWebTP(d.touchPoint) && (
        <Panel title="ENABLED FEATURES" open={open.features} onToggle={() => toggle("features")} summary={featuresSummary} badge={<InfoTip size={14} text="Defaults inherited by every display of this type, including each feature's own settings. A display can override any of them on its Enabled Features tab." />}>
        ...
        {/* ---- MULTI-ZONE LAYOUT ---- */}
        {!isWebTP(d.touchPoint) && (
        <Panel title="MULTI-ZONE LAYOUT" open={open.zones} onToggle={() => toggle("zones")} summary={zonesSummary}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <span style={{ fontSize: 13.5, display: "inline-flex", alignItems: "center", gap: 5 }}>Enable zones<InfoTip size={14} text="Each zone runs its own playlist, so each has its own rotation." /></span>
        ...
        <SaveBar dirty={dirty} onSave={saveAll} onCancel={cancelAll} />
        {delType && (() => {
          const assigned = displays.filter((x) => x.typeId === delType.id);
          const doDelete = () => {
            const keep = (list) => list.filter((x) => x.id !== delType.id);
            saveTypes(keep(savedTypes)); setTypes(keep(types));
            if (sel === delType.id) selectRaw(keep(types)[0]?.id);
            setDelType(null);
          };
          return (
            <ConfirmModal title={`Delete ${delType.name || "this display type"}?`} onCancel={() => setDelType(null)} onConfirm={doDelete}
              blocked={assigned.length ? "Displays are assigned to this display type" : null}>
              {assigned.length ? (
                <>
                  <div style={{ padding: "9px 12px", borderRadius: 6, background: "rgba(250,173,20,0.10)", border: "1px solid rgba(250,173,20,0.45)", marginBottom: 10 }}>
                    <b>This display type can't be deleted.</b> {assigned.length} display{assigned.length > 1 ? "s are" : " is"} assigned to it. Remove {assigned.length > 1 ? "them" : "it"} from the platform, or assign {assigned.length > 1 ? "them" : "it"} to another display type, first.
                  </div>
                  <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, maxHeight: 180, overflowY: "auto" }}>
                    {assigned.map((x, i) => (
                      <div key={x.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", fontSize: 12.5, borderBottom: i < assigned.length - 1 ? `1px solid ${T.borderSubtle}` : "none" }}>
                        <Icon name="tv" size={15} style={{ color: T.muted }} /><b>{x.name}</b><span style={{ color: T.muted }}>· {x.store}</span>
                      </div>
                    ))}
                  </div>
                </>
...
function PlaylistManagement({ playlists, setPlaylists, types, goToType }) {
  ...
  return (
    <div>
            ...
            <div key={p.id} style={{ borderBottom: `1px solid ${T.borderSubtle}` }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.7fr 1.5fr 110px", alignItems: "center", fontSize: 13, background: isSel ? T.primaryTint : "transparent" }}>
                ...
                <div style={{ padding: "6px 12px" }}>
                  <Btn variant="danger" title="Delete playlist"
                    style={{ height: 28, padding: "0 10px", fontSize: 12.5 }} onClick={() => setConfirmDel(p.id)}>
      ...
      {confirmDel && (() => {
        const p = playlists.find((x) => x.id === confirmDel);
        if (!p) return null;
        const u = usage[p.id] || [];
        return (
          <ConfirmModal title={`Delete ${p.name}?`} onCancel={() => setConfirmDel(null)} onConfirm={() => remove(p)}
            blocked={u.length ? "This playlist is assigned to a display type or zone" : null}>
            {u.length ? (
              <>
                <div style={{ padding: "9px 12px", borderRadius: 6, background: "rgba(250,173,20,0.10)", border: "1px solid rgba(250,173,20,0.45)", marginBottom: 10 }}>
                  <b>This playlist can't be deleted.</b> It is assigned to {u.length} display type{u.length > 1 ? "s or zones" : " or zone"}. Reassign {u.length > 1 ? "them" : "it"} first.
                </div>
                <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, maxHeight: 180, overflowY: "auto" }}>
                  {u.map((x, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", fontSize: 12.5, borderBottom: i < u.length - 1 ? `1px solid ${T.borderSubtle}` : "none" }}>
                      <Icon name="dashboard_customize" size={15} style={{ color: T.muted }} /><b>{x.type.name}</b><span style={{ color: T.muted }}>· {x.where}</span>
                    </div>
                  ))}
                </div>
              </>
