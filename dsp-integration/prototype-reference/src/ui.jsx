...
export const SectionLabel = ({ children, style, tip }) => (
  <div style={{ fontSize: 12, letterSpacing: "0.5px", textTransform: "uppercase", color: T.muted, marginTop: 24, marginBottom: 12, display: "flex", alignItems: "center", gap: 6, ...style }}>{children}{tip && <InfoTip text={tip} size={14} />}</div>
);
...
export const Fld = ({ label, children, hint, required }) => (
  <div style={{ minWidth: 0 }}>
    <div style={{ fontSize: 12, color: T.muted, marginBottom: 5, display: "flex", alignItems: "center", gap: 5, minWidth: 0 }}>
      <span style={{ minWidth: 0 }}>{required && <span style={{ color: T.error }}>* </span>}{label}</span>
      {hint && <InfoTip text={hint} size={14} />}
    </div>
    {children}
  </div>
);
...
/* Save / cancel bar for an editable page. Sticky to the bottom of the
   content column so it is always visible; Save changes is enabled only
   once something on the page has actually changed. */
export const SaveBar = ({ dirty, onSave, onCancel }) => (
  <div style={{ position: "sticky", bottom: 0, zIndex: 5, marginTop: 24, padding: "12px 0", background: "#fff", borderTop: `1px solid ${T.borderSubtle}`, display: "flex", alignItems: "center", gap: 8 }}>
    <span style={{ flex: 1, fontSize: 12.5, color: dirty ? T.warning : T.micro, display: "inline-flex", alignItems: "center", gap: 6 }}>
      <Icon name={dirty ? "edit_note" : "check"} size={16} />{dirty ? "You have unsaved changes." : "No changes to save."}
    </span>
    <Btn onClick={onCancel} disabled={!dirty}>Cancel</Btn>
    <Btn variant="primary" onClick={onSave} disabled={!dirty}>Save changes</Btn>
  </div>
);

/* Info icon whose tooltip explains the field or section it sits next to.
   Fixed-positioned so it is never clipped by a panel or table. */
export function InfoTip({ text, size = 15 }) {
  const [pos, setPos] = useState(null);
  const show = (e) => {
    const r = e.currentTarget.getBoundingClientRect();
    const w = 280, vw = window.innerWidth || 1200;
    const left = Math.min(Math.max(8, r.left + r.width / 2 - w / 2), vw - w - 8);
    const below = r.top < 200;   // not enough room above: open below instead
    setPos({ left, top: below ? r.bottom + 8 : r.top - 8, below, w });
  };
  return (
    <span tabIndex={0} aria-label={typeof text === "string" ? text : undefined}
      ...
      <Icon name="info" size={size} style={{ color: T.micro }} />
      {pos && (
        <span role="tooltip" style={{ position: "fixed", left: pos.left, top: pos.top, transform: pos.below ? "none" : "translateY(-100%)", zIndex: 60, width: "max-content", maxWidth: pos.w,
          padding: "7px 10px", borderRadius: 6, background: "rgba(0,0,0,0.86)", color: "#fff", fontSize: 11.5, fontWeight: 400, lineHeight: 1.45,
          letterSpacing: 0, textTransform: "none", whiteSpace: "normal", textAlign: "left", pointerEvents: "none" }}>{text}</span>
      )}
    </span>
  );
}
