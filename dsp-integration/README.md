...
| `apps/admin/src/features/display-types/` | Display Types screen: list, form, panels, delete. `panels/PlaylistSettingsFields.tsx` (the old Playlist Settings panel's fields, no longer a panel of its own here — see Playlist Management below) |
| `apps/admin/src/features/playlist-management/` | Playlist Management screen: rename and delete, plus — moved from Display Types, 26 Sep 2026 — each playlist's own Playlist Settings, expanded under its row (one settings block per assignment when shared), including slot assignment (Advertiser greyed out for a new slot while DSP integration is switched off) |
