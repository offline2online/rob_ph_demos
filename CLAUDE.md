...
- **Confirmed directly (1 Sep 2026): day-to-day HQ/Retail Admin pricing work — the "Products & Pricing" product editor (tabs: Product Assets / Product Details / Pricing / Stockists) and the store-level "Menu Boards" list where a store sets its own local offer price — happens on the real Personalisation Hub platform's own native pages, NOT via this repo's `hq-admin.html`/`retail-admin.html`.** The giveaway that generalises: real-platform pricing copy talks about "Badge for External Displays" targeting — a concept that doesn't exist anywhere in this repo (verified by grep) — so any screen using that phrasing, or otherwise not matching this repo's actual copy/markup, is platform-native and out of this repo's reach to fix. A pricing/offer bug reported from one of those screens is a real-platform bug: the most this repo's code can do is explain the *data model* (e.g. why an offer priced exactly at RRP shows no visible discount — that rule is intentional and consistent everywhere) so the user can find and correct the right field on the real platform; it is not something to patch here. Only `menu-board.html` (the customer-facing display board itself) has been confirmed as genuinely this repo's own code in live use — don't assume the same is still true of `hq-admin.html`/`retail-admin.html` for pricing/admin workflows without checking copy against a screenshot first.
