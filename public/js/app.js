...
function projectSectionHTML(project) {
  ...
  const nameRow = `<div class="project-name-row"><h2 class="project-name">${escapeHTML(project.name)} <span class="project-item-count">(${total})</span></h2>
         <button type="button" class="project-rename-btn" data-project-id="${escapeHTML(project.id)}" title="Edit project">&#9998;</button>
       </div>`;
...
projectsRoot.addEventListener("click", (e) => {
  ...
  if (renameBtn) { openEditProjectModal(renameBtn.dataset.projectId); return; }
...
// ── Edit Project modal — reached via a project header's pencil icon.
// Same "plain sheet" styling as the New Project modal (.np-panel), but for
// an existing project: lets both its name and its Program/Product grouping
// be corrected in one place, replacing the old inline rename-only text
// field (which had no way to touch programId at all). ─────────────────────
const epBackdrop = document.getElementById("ep-backdrop");
const epProgramSelect = document.getElementById("ep-program-select");
wireProgramSelect(epProgramSelect);
let epProjectId = null;

function openEditProjectModal(pid) {
  const project = projects.find((p) => p.id === pid);
  if (!project) return;
  epProjectId = pid;
  epBackdrop.hidden = false;
  document.getElementById("ep-name-input").value = project.name;
  populateProgramSelect(epProgramSelect, project.programId || "");
  document.getElementById("ep-name-input").focus();
}
function closeEditProjectModal() { epBackdrop.hidden = true; epProjectId = null; }

document.getElementById("ep-cancel").addEventListener("click", closeEditProjectModal);
document.getElementById("ep-close").addEventListener("click", closeEditProjectModal);
epBackdrop.addEventListener("click", (e) => { if (e.target === epBackdrop) closeEditProjectModal(); });
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && !epBackdrop.hidden) closeEditProjectModal();
});
document.getElementById("ep-submit").addEventListener("click", async () => {
  const nameEl = document.getElementById("ep-name-input");
  const name = nameEl.value.trim();
  if (!name) { nameEl.focus(); return; }
  const programId = epProgramSelect.value !== "__new__" ? epProgramSelect.value : "";
  const pid = epProjectId;
  await Promise.all([setProjectName(pid, name), setProjectProgram(pid, programId)]);
