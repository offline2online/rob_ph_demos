...
/* ------------------------------ partner DSPs ------------------------------
   An advertiser slot is only sellable if demand can actually reach it. Demand
   arrives through a partner DSP, so the DSP is the connection and the seats on
   it are the advertisers a slot can be reserved to. Credentials live here, per
   partner — never on the slot, and never on the display type.

   "__direct__" is the house book: advertisers sold direct, with no DSP in the
   path. It always exists and cannot be disconnected.
-------------------------------------------------------------------------- */
const DIRECT_PARTNER = "__direct__";
const ANY_PARTNER = "__any__";
const RTB = "__rtb__";
...
/* Slot table columns: #, Label, Owner, Assigned to. The form column this sits
   ...
   stacked, rather than each taking a column the panel cannot afford. */
const SLOT_GRID = "28px 1fr 100px 144px";

const AUCTION_TYPES = ["Open RTB", "Preferred deal", "Programmatic guaranteed"];
const CURRENCIES = ["GBP", "EUR", "USD", "AUD"];
const IAB_CATEGORIES = ["Food & Drink", "Health & Fitness", "Beauty", "Retail", "Family & Parenting", "Automotive", "Finance", "Travel"];
...
const INITIAL_PARTNERS = [
  ...
  { id: "p_amazon", provider: "amazon_dsp", name: "Amazon Ads DSP", status: "error",
    creds: { region: "Europe (EU)", clientId: "amzn1.application-oa2-client.7f3c", clientSecret: "\u2022".repeat(16),
             refreshToken: "\u2022".repeat(16), profileId: "3390127745", advertiserId: "ENTITY4K2M9P", entityId: "ENTITY8Q1R5T" },
...
const providerOf = (p) => (p && p.provider ? DSP_PROVIDERS[p.provider] : null);
const partnerColour = (p) => (providerOf(p) ? providerOf(p).colour : "#7c3aed");

/* Missing required credentials are what keep a partner out of the slot picker. */
const missingCreds = (provider, creds) => {
  const def = DSP_PROVIDERS[provider];
  if (!def) return [];
  return def.fields
    .filter((f) => f.required && (!f.when || f.when(creds || {})))
...
/* A DSP's advertiser universe is not enumerable from our side, so the lists
   hold whatever the client names — known seats or free-typed advertisers. */
const isDsp = (p) => !!(p && p.provider);
...
function caps(d) {
  ...
  const p = webEl(d.elementType).plays;
  return {
...
const STRUCTURE_MARKERS = [
  { key: "phantom", icon: "crop_free", label: "Phantom zone defined", test: (t) => t.phantomEnabled },
  { key: "zones", icon: "grid_view", label: "Multi-zone layout", test: (t) => t.zonesEnabled },
  { key: "slots", icon: "view_week", label: "Capped rotation with assigned slots", test: (t) => t.maxRotation && t.maxRotation !== "Unlimited" },
];
...
const BLANK_FEATURES = () => ({
  in_store_radio: { on: false },
  qr_control: { on: false, size: 100, colour: "#000000", connectedColour: "#169bc2", qrPosition: "Center",
    mobileTemplate: "Mobile App", overlayPosition: "Bottom Right", offsetX: 24, offsetY: 24 },
  proximity_mist: { on: false, mode: "zone", zone: MIST_ZONES[0] },
  ai_agent_playback: { on: false },
  vision_ai: { on: false, mode: VISION_MODES[0], shopfront: false, streamQuality: 640, fps: 15, frameSkip: 5, missThreshold: 15 },
});

const DEFAULTS = {
  assetPosition: "Top-Left", assetFill: "Fit to Display", maxRotation: "Unlimited",
  transition: "None", autoRotation: "Auto-Rotate On", autoPlay: "Auto-Play On",
  phantomPosition: "Bottom Right",
};

const INITIAL_PLAYLISTS = [
  { id: "pl_landscape", name: "Landscape Playlist", autoFor: "landscape" },
  { id: "pl_portrait", name: "Portrait Playlist", autoFor: "portrait" },
  { id: "pl_menu", name: "Menu Board Playlist", autoFor: "menu_board" },
  { id: "pl_web_hero", name: "Web Hero Playlist", autoFor: "web_hero" },
  { id: "pl_mss_default", name: "Default Mobile Store Site Playlist", autoFor: null },
  { id: "pl_zone_menu_board_1", name: "Menu Board — Long Format / Zone 1", autoFor: "menu_board" },
  { id: "pl_zone_menu_board_2", name: "Menu Board — Long Format / Zone 2", autoFor: "menu_board" },
  { id: "pl_zone_menu_board_3", name: "Menu Board — Long Format / Zone 3", autoFor: "menu_board" },
  { id: "pl_promo", name: "Promo Rotation", autoFor: null },
  { id: "pl_notices", name: "Store Notices", autoFor: null },
  { id: "pl_seasonal", name: "Seasonal Overflow", autoFor: null },
  { id: "pl_archive", name: "Archived Q1 Campaigns", autoFor: null },
];

const baseType = (over) => ({
  touchPoint: "Digital Signage", elementType: "hero", description: "", background: "#000000",
  widthMode: "contained", maxWidth: 1200, cols: 3, colsTablet: 2, colsMobile: 1, gap: 16, itemAspect: "4:3",
  assetPosition: null, assetFill: null, maxRotation: null, transition: null, autoRotation: null, autoPlay: null,
  phantomEnabled: false, phantomW: 250, phantomH: 250, phantomPosition: null, phantomSizing: "Fit to Display",
  features: BLANK_FEATURES(), zonesEnabled: false, zones: [], slots: [],
  qrIdle: { qrColour: "#000000", qrSize: 120, showPoweredBy: true, poweredByText: "Powered by Personalisation Hub" },
  qrConnected: { icon: "smartphone", iconColour: "#169bc2", iconFile: CONNECTED_ASSET_NAME, showPoweredBy: true },
  ...over,
});

const INITIAL_TYPES = [
  baseType({ id: "landscape", name: "Landscape", w: 1920, h: 1080, defaultPlaylist: "pl_landscape",
    phantomEnabled: true,
    features: { ...BLANK_FEATURES(), qr_control: { ...BLANK_FEATURES().qr_control, on: true }, vision_ai: { ...BLANK_FEATURES().vision_ai, on: true } } }),
  baseType({ id: "portrait", name: "Portrait", w: 1080, h: 1920, defaultPlaylist: "pl_portrait",
    phantomEnabled: true, phantomW: 220, phantomH: 220,
    features: { ...BLANK_FEATURES(), qr_control: { ...BLANK_FEATURES().qr_control, on: true, mobileTemplate: "Mobile Store Site" }, proximity_mist: { on: true, mode: "zone", zone: "Front of Store" } } }),
  baseType({ id: "menu_board", name: "Menu Board — Long Format", w: 5760, h: 1080, defaultPlaylist: "pl_menu",
    background: "#111111", maxRotation: "3", phantomEnabled: true,
    slots: [
      { label: "Priority 1", owner: "internal", advertiser: null },
      { label: "Supplier slot", owner: "advertiser", partnerId: "p_google", advertiser: RTB },
      { label: "Store choice", owner: "retail", advertiser: null, storeScope: "Store staff" },
    ],
    features: { ...BLANK_FEATURES(), qr_control: { ...BLANK_FEATURES().qr_control, on: true, mobileTemplate: "Order & Pay" }, vision_ai: { ...BLANK_FEATURES().vision_ai, on: true } },
    zonesEnabled: true,
    zones: [
      { id: "z1", name: "Zone 1", x: 0, y: 0, w: 33.3, h: 100, playlist: "pl_zone_menu_board_1" },
      { id: "z2", name: "Zone 2", x: 33.3, y: 0, w: 33.4, h: 100, playlist: "pl_zone_menu_board_2" },
      { id: "z3", name: "Zone 3", x: 66.7, y: 0, w: 33.3, h: 100, playlist: "pl_zone_menu_board_3" },
    ] }),
  baseType({ id: "web_hero", name: "Hero Banner", touchPoint: "Responsive Web", elementType: "hero", nameAuto: true, ...applyElementDefaults("hero"), w: 1200, h: 520, defaultPlaylist: "pl_web_hero",
    maxRotation: "1", phantomEnabled: false, slots: [{ label: "Featured offer", owner: "internal", advertiser: null }] }),
  baseType({ id: "web_carousel", name: "Carousel", touchPoint: "Responsive Web", elementType: "carousel", nameAuto: true, ...applyElementDefaults("carousel"), w: 1200, h: 360, defaultPlaylist: "pl_promo",
    phantomEnabled: false, slots: [
      { label: "Priority 1", owner: "internal", advertiser: null },
      { label: "Supplier slot", owner: "advertiser", partnerId: ANY_PARTNER, advertiser: RTB },
      { label: "Supplier slot", owner: "advertiser", partnerId: DIRECT_PARTNER, advertiser: "Blackmores" },
      { label: "Store choice", owner: "retail", advertiser: null, storeScope: "Store staff" }] }),
  baseType({ id: "web_grid", name: "Grid", touchPoint: "Responsive Web", elementType: "grid", nameAuto: true, ...applyElementDefaults("grid"), w: 1200, h: 640, defaultPlaylist: "pl_promo",
    phantomEnabled: false, slots: Array.from({ length: 6 }, (_, i) => ({ label: `Tile ${i + 1}`, owner: "internal", advertiser: null })) }),
  baseType({ id: "web_product", name: "Product Tiles", touchPoint: "Responsive Web", elementType: "product", nameAuto: true, ...applyElementDefaults("product"), w: 1200, h: 180, defaultPlaylist: "pl_web_hero",
    maxRotation: "1", phantomEnabled: false, slots: [{ label: "PH authoritative", owner: "internal", advertiser: null }] }),
  baseType({ id: "web_qr", name: "QR Control", touchPoint: "Responsive Web", elementType: "qr_control", nameAuto: true, ...applyElementDefaults("qr_control"), w: 240, h: 240, defaultPlaylist: "pl_web_hero",
    maxRotation: "1", phantomEnabled: false, slots: [] }),
  baseType({ id: "web_ctas", name: "CTAs", touchPoint: "Responsive Web", elementType: "ctas", nameAuto: true, ...applyElementDefaults("ctas"), w: 1200, h: 260, defaultPlaylist: "pl_notices",
    phantomEnabled: false, slots: Array.from({ length: 3 }, (_, i) => ({ label: `Action ${i + 1}`, owner: "internal", advertiser: null })) }),
];
...
function usageOf(playlistId, types) {
  const out = [];
  types.forEach((t) => {
    if (t.defaultPlaylist === playlistId) out.push({ type: t, where: "Default playlist" });
    if (t.zonesEnabled) t.zones.forEach((z) => { if (z.playlist === playlistId) out.push({ type: t, where: z.name }); });
  });
  return out;
}
...
/* ------------------------------ partners / DSPs ---------------------------
   Where demand comes from. A partner is a connection: credentials, the seats
   (advertisers) that connection can sell to, and the floor/category rules that
   apply to anything it wins. Display types consume this list — they never hold
...
function PartnersView({ partners, setPartners, companyLists, setCompanyLists, types, goToType }) {
  ...
  /* Every display-type position currently pointing at this partner. */
  const usage = useMemo(() => {
    const out = [];
    (types || []).forEach((t) => (t.slots || []).forEach((sl, i) => {
      if (sl.owner === "advertiser" && (sl.partnerId || ANY_PARTNER) === p.id) {
        out.push({ typeId: t.id, typeName: t.name, touchPoint: t.touchPoint, slot: i + 1, label: sl.label, advertiser: sl.advertiser });
      }
    }));
    return out;
  }, [types, p.id]);
  ...
  const addPartner = (providerKey) => {
    ...
    const id = `p_${providerKey}_${Date.now().toString(36)}`;
  ...
  /* Disconnecting keeps the record — the positions pointing at it must stay
     visible and fixable rather than silently reverting to Headquarters. */
              ...
              <div key={x.id} onClick={() => setSel(x.id)}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: T.micro }}>{pr ? pr.sub : "No DSP in the path"}</div>
                  </div>
          ...
          {Object.entries(DSP_PROVIDERS).map(([k, def]) => (
            <div key={k} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", marginBottom: 6,
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: T.micro }}>{def.sub}</div>
              </div>
              <Btn variant="text" style={{ height: 26, fontSize: 12, padding: "0 8px" }}
                disabled={alreadyAdded(k)} title={alreadyAdded(k) ? "Already added" : undefined}
                onClick={() => (alreadyAdded(k) ? null : setAdding(k))}>
                {alreadyAdded(k) ? "Added" : "Add"}
              </Btn>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {onCompany ? (
            ...
            <>
                ...
                <div style={{ flex: 1, minWidth: 200 }}>
                  {p.system ? (
                    ...
                    <input value={p.name} onChange={(e) => setP({ name: e.target.value })}
                      style={{ ...inputStyle, fontSize: 16, fontWeight: 600, height: 34 }} />
                  )}
                  <div style={{ fontSize: 12, color: T.muted, marginTop: 4 }}>
                    {prov ? prov.sub : "Advertisers sold direct, with no DSP in the path."}
                    {p.lastSync && <> &middot; {p.lastSync}</>}
                  </div>
                </div>
                <Pill color={STATUS_STYLE[p.status].colour} bg="#fff" border={STATUS_STYLE[p.status].colour}>
                  <Icon name={STATUS_STYLE[p.status].icon} size={13} />{STATUS_STYLE[p.status].label}
                </Pill>
              </div>

              {p.system ? (
                <div style={{ marginTop: 16, padding: 12, borderRadius: 6, background: T.surfaceAlt, border: `1px solid ${T.borderSubtle}`, fontSize: 12.5, color: T.muted, lineHeight: 1.6 }}>
                  The house book. Advertisers sold direct by the commercial team, with no DSP and no auction —
                  a position reserved here is filled from that advertiser's own campaigns. It has no credentials
                  to configure and cannot be disconnected.
                </div>
              ) : (
                <>
                  {/* ------------------------------------ credentials */}
                  <SectionLabel>Connection credentials</SectionLabel>
                  ...
                  <Grid cols={2}>
                    {prov.fields.filter((f) => !f.when || f.when(p.creds)).map((f) => (
                      <Fld key={f.key} label={<>{f.required && <span style={{ color: T.error }}>*</span>}{f.label}</>} hint={f.hint}>
                        {f.type === "select" ? (
                          <select value={p.creds[f.key] || ""} onChange={(e) => setCred(f.key, e.target.value)} style={ctl}>
                            {f.options.map((o) => <option key={o}>{o}</option>)}
                          </select>
                        ) : f.multiline ? (
                          <textarea value={p.creds[f.key] || ""} onChange={(e) => setCred(f.key, e.target.value)}
                            placeholder={f.placeholder} rows={3}
                            style={{ ...ctl, height: "auto", fontFamily: MONO, fontSize: 12, resize: "vertical" }} />
                        ) : (
                          <div style={{ position: "relative" }}>
                            <input
                              type={f.secret && !reveal[f.key] ? "password" : "text"}
                              value={p.creds[f.key] || ""} onChange={(e) => setCred(f.key, e.target.value)}
                              placeholder={f.placeholder}
                              style={{ ...ctl, paddingRight: f.secret ? 34 : 11, fontFamily: f.secret ? MONO : FONT }} />
                            {f.secret && (
                              <Icon name={reveal[f.key] ? "visibility_off" : "visibility"} size={16}
                                onClick={() => setReveal({ ...reveal, [f.key]: !reveal[f.key] })}
                                style={{ position: "absolute", right: 9, top: 8, color: T.micro, cursor: "pointer" }} />
                            )}
                          </div>
                        )}
                      </Fld>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap", alignItems: "center" }}>
                    <Btn variant="primary" disabled={!canConnect} onClick={connect}
                      title={canConnect ? undefined : `Missing: ${missing.join(", ")}`}>
                      <Icon name="link" size={16} />{p.status === "connected" ? "Re-test connection" : "Connect"}
                    </Btn>
                    {p.status !== "draft" && <Btn onClick={disconnect}><Icon name="link_off" size={16} />Disconnect</Btn>}
                    {!canConnect && (
                      <span style={{ fontSize: 12, color: T.error, display: "inline-flex", alignItems: "center", gap: 5 }}>
                        <Icon name="error" size={14} />Missing {missing.join(", ")}
                      </span>
                    )}
                  </div>
                  <Note>
                  ...
                  <Grid cols={3}>
                    <Fld label="Auction type">
                      <select value={p.auctionType} onChange={(e) => setP({ auctionType: e.target.value })} style={ctl}>
                        {AUCTION_TYPES.map((a) => <option key={a}>{a}</option>)}
                      </select>
                    </Fld>
                    <Fld label="Floor price (CPM)" hint="Bids below the floor never win the position.">
                      <input type="number" step="0.10" value={p.floorCpm ?? ""} onChange={(e) => setP({ floorCpm: e.target.value === "" ? null : Number(e.target.value) })}
                        placeholder="No floor" style={ctl} />
                    </Fld>
                    <Fld label="Currency">
                      <select value={p.currency} onChange={(e) => setP({ currency: e.target.value })} style={ctl}>
                        {CURRENCIES.map((c) => <option key={c}>{c}</option>)}
                      </select>
                    </Fld>
                  </Grid>

                  <div style={{ marginTop: 14 }} /><Fld label="Permitted categories" hint="Empty means every category this partner offers is eligible.">
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {IAB_CATEGORIES.map((c) => {
                        const on = p.categories.includes(c);
                        const blocked = p.exclusions.includes(c);
                        return (
                          <span key={c} onClick={() => setP({ categories: on ? p.categories.filter((x) => x !== c) : [...p.categories, c] })}
                            style={{ cursor: blocked ? "not-allowed" : "pointer", opacity: blocked ? 0.35 : 1,
                                     display: "inline-flex", alignItems: "center", gap: 4, height: 24, padding: "0 10px", borderRadius: 9999,
                                     fontSize: 12, border: `1px solid ${on ? T.primary : T.border}`,
                                     background: on ? T.primaryTint : "#fff", color: on ? T.primary : T.text }}>
                  ...
                  <div style={{ marginTop: 14 }} /><Fld label="Competitive exclusions" hint="Blocked outright, whatever the bid. Overrides the permitted list.">
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {IAB_CATEGORIES.map((c) => {
                        const on = p.exclusions.includes(c);
                        return (
                          <span key={c} onClick={() => setP({
                            exclusions: on ? p.exclusions.filter((x) => x !== c) : [...p.exclusions, c],
                            categories: on ? p.categories : p.categories.filter((x) => x !== c),
                          })}
                            style={{ cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4, height: 24, padding: "0 10px",
                                     borderRadius: 9999, fontSize: 12, border: `1px solid ${on ? T.error : T.border}`,
                                     background: on ? "rgba(255,77,79,0.08)" : "#fff", color: on ? T.error : T.text }}>
                            {on && <Icon name="block" size={13} />}{c}
                          </span>
                        );
                      })}
                    </div>
              ...
              {p.status !== "connected" && !p.system ? (
                <div style={{ padding: 12, borderRadius: 6, background: T.surfaceAlt, border: `1px solid ${T.borderSubtle}`, fontSize: 12.5, color: T.muted }}>
                  Advertisers are pulled from the partner on connect. Connect first, or the slot picker has nothing
                  to reserve a position to.
                </div>
              ) : (
                <>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                    {p.seats.map((a) => (
                      <Pill key={a.id} color={T.text} bg="#fff" border={T.border}>
                        <Icon name="sell" size={13} style={{ color: partnerColour(p) }} />{a.name}
                      </Pill>
                    ))}
                    {p.seats.length === 0 && <span style={{ fontSize: 12.5, color: T.muted }}>None yet.</span>}
                  </div>
                  <Btn variant="text" style={{ paddingLeft: 0 }}
                    onClick={() => setP({ seats: [...p.seats, { id: `s${Date.now().toString(36)}`, name: `Advertiser ${p.seats.length + 1}` }] })}>
                    <Icon name="add" size={16} />Add advertiser
                  </Btn>
                  <Note>
                    These are what the <b>Assigned to</b> picker offers once a position names this partner. An
                    advertiser belongs to one partner, so moving a position to a different partner clears the
                    reservation rather than carrying a name that partner cannot serve.
                  </Note>
                </>
              )}
              ...
              {usage.length === 0 ? (
                <div style={{ padding: 12, borderRadius: 6, background: T.surfaceAlt, border: `1px solid ${T.borderSubtle}`, fontSize: 12.5, color: T.muted }}>
                  No display type has an advertiser position pointing here yet. Cap a rotation on a display type,
                  set a position's owner to <b>Advertiser</b>, then name this partner.
                </div>
              ) : (
                <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 130px 60px 1fr 90px", background: T.surfaceAlt, borderBottom: `1px solid ${T.borderSubtle}`, fontSize: 12, fontWeight: 600, color: T.muted }}>
                    {["Display type", "Touch point", "Slot", "Position", ""].map((h, i) => <div key={i} style={{ padding: "7px 10px" }}>{h}</div>)}
                  </div>
                  {usage.map((u, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 130px 60px 1fr 90px", alignItems: "center", fontSize: 12.5,
                                          borderBottom: i < usage.length - 1 ? `1px solid ${T.borderSubtle}` : "none" }}>
                      <div style={{ padding: "8px 10px" }}>{u.typeName}</div>
                      <div style={{ padding: "8px 10px", color: T.muted, display: "flex", alignItems: "center", gap: 5 }}>
                        <Icon name={tpIcon(u.touchPoint)} size={14} />{u.touchPoint}
                      </div>
                      <div style={{ padding: "8px 10px", color: T.micro }}>{u.slot}</div>
                      <div style={{ padding: "8px 10px" }}>
                        {u.label}
                        <span style={{ color: !u.advertiser || u.advertiser === RTB ? "#7c3aed" : T.muted, marginLeft: 6 }}>
                          {!u.advertiser || u.advertiser === RTB ? "RTB" : u.advertiser}
                        </span>
                      </div>
                      <div style={{ padding: "4px 10px" }}>
                        <Btn variant="text" style={{ height: 26, fontSize: 12, padding: 0 }} onClick={() => goToType(u.typeId)}>Open</Btn>
                      </div>
                    </div>
                  ))}
                </div>
...
function ListEditor({ label, tone, icon, items, suggestions, onAdd, onRemove, empty, readOnly }) {
  ...
  /* Adding touches both lists, so the parent owns the write — doing it here as
     two calls would read the same stale partner twice and lose the first. */
  const add = (name) => {
    const n = (name || "").trim();
    if (!n || has(n)) return;
    onAdd(n);
    ...
    <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, padding: 12,
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <input value={draft} onChange={(e) => setDraft(e.target.value)}
          ...
          placeholder="Add an advertiser…" style={{ ...ctl, height: 28, fontSize: 12.5 }} />
      ...
      {!readOnly && unused.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 8, alignItems: "center" }}>
          ...
          {unused.map((sg) => (
            <span key={sg.id} onClick={() => add(sg.name)}
              style={{ cursor: "pointer", fontSize: 11.5, height: 20, padding: "0 8px", borderRadius: 9999,
                       border: `1px dashed ${T.border}`, color: T.muted, display: "inline-flex", alignItems: "center" }}>
              + {sg.name}
            </span>
          ))}
        </div>
...
/* The add flow is one confirm step — credentials are entered on the partner
   itself afterwards, so a half-filled connection is a visible "Not connected"
   record rather than a modal someone abandoned. */
  ...
  return (
    <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 8, overflow: "hidden" }}>
      <div style={{ padding: "12px 16px", background: T.surfaceAlt, borderBottom: `1px solid ${T.borderSubtle}`, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 500 }}>Add {def.label}</div>
      ...
      <div style={{ padding: 16 }}>
        ...
        <ul style={{ margin: 0, paddingLeft: 18, fontSize: 13, lineHeight: 1.9, color: T.text }}>
          {def.fields.filter((f) => f.required).map((f) => <li key={f.key}>{f.label}</li>)}
        </ul>
        <div style={{ display: "flex", gap: 8, marginTop: 18 }}>
          <Btn variant="primary" onClick={onAdd}><Icon name="add" size={16} />Add partner</Btn>
          <Btn onClick={onCancel}>Cancel</Btn>
        </div>
        <Note>
          The partner is created as <b>Not connected</b>. It stays out of the slot picker's reserved list until
          the credentials test, so a position can never be sold to demand that cannot be delivered.
        </Note>
      </div>
    </div>
  );
...
function TypesView({ types, setTypes, playlists, setPlaylists, sel, setSel, templates, partners, companyLists, goToPartners }) {
  ...
  const set = (patch) => setTypes(types.map((t) => (t.id === sel ? { ...t, ...patch } : t)));
  ...
  const advertiserSlots = (d?.slots || []).filter((sl) => sl.owner === "advertiser");
  ...
  const askChangeElement = (k) => { if (k !== d.elementType) setPendingEl(k); };
  ...
  const ensureZonePlaylist = (n) => {
    ...
    setPlaylists((prev) => (prev.some((p) => p.id === id) ? prev : [...prev, { id, name, autoFor: d.id }]));
    return id;
  };
    ...
    <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
      <div style={{ width: 215, flexShrink: 0 }}>
        <Btn variant="primary" style={{ width: "100%", justifyContent: "center", marginBottom: 10 }}
          onClick={() => {
            ...
            const pid = `pl_${id}`;
            setPlaylists([...playlists, { id: pid, name: "New Display Type Playlist", autoFor: id }]);
            setTypes([...types, baseType({ id, name: "", nameAuto: false, touchPoint: "Digital Signage",
              w: 1920, h: 1080, defaultPlaylist: pid, maxRotation: "Unlimited", phantomEnabled: false, slots: [] })]);
            setSel(id);
          }}>
        ...
        <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 8, overflow: "hidden" }}>
          {types.map((t) => {
            ...
            return <div key={t.id} onClick={() => setSel(t.id)}
              ...
              <div style={{ fontSize: 11, color: T.muted, marginTop: 3, marginLeft: 24 }}>{t.w}×{t.h}</div>
              <div style={{ display: "flex", gap: 5, marginTop: 5, marginLeft: 24, flexWrap: "wrap" }}>
                ...
                {FEATURES.filter((f) => COMPANY_AVAILABILITY[f.key] && t.features[f.key].on).map((f) => (
      ...
      <div style={{ width: 400, flexShrink: 0 }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <IconSelect value={d.touchPoint} onChange={(v) => set({ touchPoint: v })} options={TOUCH_POINTS} />
          {isWebTP(d.touchPoint) && (
            <div style={{ marginTop: 14 }}>
              <Label required>Element Type</Label>
              <select value={d.elementType}
              ...
              <div style={{ padding: 10, marginTop: 8, borderRadius: 6, background: T.surfaceAlt, border: `1px solid ${T.borderSubtle}`, fontSize: 11.5, color: T.muted, lineHeight: 1.5 }}>
                {webEl(d.elementType).desc}
                <div style={{ marginTop: 5, color: T.micro }}>
                  Plays: <b style={{ color: T.text }}>{webEl(d.elementType).plays}</b>
                  {" · "}
                  {caps(d).campaigns
                ...
                <div style={{ marginTop: 10, padding: 11, borderRadius: 6, background: "rgba(250,173,20,0.10)", border: `1px solid rgba(250,173,20,0.4)`, fontSize: 12.5, lineHeight: 1.5 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <Btn variant="primary" style={{ height: 28, fontSize: 12.5 }}
                      onClick={() => { set({ elementType: pendingEl, ...applyElementDefaults(pendingEl), ...(d.nameAuto !== false ? { name: webEl(pendingEl).name, nameAuto: true } : {}) }); setPendingEl(null); }}>
                      Change and reset
                    </Btn>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input value={d.name} autoFocus={!d.name} placeholder="Name this display type / element"
            onChange={(e) => set({ name: e.target.value, nameAuto: false })}
            style={{ ...inputStyle, borderColor: d.name ? T.border : T.warning }} />
          {isWebTP(d.touchPoint) && d.nameAuto !== false && (
        ...
        {!isWebTP(d.touchPoint) && (
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input type="number" value={d.w} onChange={(e) => set({ w: Number(e.target.value) })} style={{ ...inputStyle, width: 95 }} />
              <span style={{ color: T.muted }}>H</span>
              <input type="number" value={d.h} onChange={(e) => set({ h: Number(e.target.value) })} style={{ ...inputStyle, width: 95 }} />
            </div>
          </div>
        )}
        <div style={{ marginBottom: 16 }}>
          <Label>Background Color</Label>
          <input type="color" value={d.background} onChange={(e) => set({ background: e.target.value })}
        ...
        {caps(d).campaigns ? (
          <div>
            ...
            <select value={d.defaultPlaylist} onChange={(e) => set({ defaultPlaylist: e.target.value })} style={inputStyle}>
              {playlists.map((p) => <option key={p.id} value={p.id}>{p.name}{p.autoFor === d.id ? " (auto-created)" : ""}</option>)}
            </select>
          </div>
        ) : (
          <div style={{ padding: 10, borderRadius: 6, background: T.surfaceAlt, border: `1px solid ${T.borderSubtle}`, fontSize: 12, color: T.muted, lineHeight: 1.5 }}>
            <b style={{ color: T.text }}>{webEl(d.elementType).name}</b> renders no campaigns, so it has no playlist.
            Its content is authored directly{d.elementType === "commercial" ? " by Personalisation Hub." : "."}
          </div>
        )}
        ...
        <Panel title="PLAYLIST SETTINGS" open={open.playlist} onToggle={() => toggle("playlist")}>
          <Row>
            <Col><Label>Asset Position</Label><DefaultSelect value={d.assetPosition} onChange={(v) => set({ assetPosition: v })} fallback={DEFAULTS.assetPosition} options={["Top-Left", "Top-Right", "Center", "Bottom-Left", "Bottom-Right"]} /></Col>
            <Col><Label>Asset Fill</Label><DefaultSelect value={d.assetFill} onChange={(v) => set({ assetFill: v })} fallback={DEFAULTS.assetFill} options={["Fit to Display", "Maintain Asset Property", "Fill", "Stretch"]} /></Col>
          </Row>
          <Row>
            <Col><Label>Maximum Campaigns Played In Rotation</Label>
              <DefaultSelect value={d.maxRotation} onChange={(v) => {
                if (!v || v === "Unlimited") return set({ maxRotation: v, slots: [] });
                const n = Number(v); const next = [...d.slots];
                while (next.length < n) next.push({ label: `Slot ${next.length + 1}`, owner: "internal", advertiser: null, storeScope: null });
                set({ maxRotation: v, slots: next.slice(0, n) });
              }} fallback={DEFAULTS.maxRotation} options={["Unlimited", "1", "2", "3", "4", "5", "6", "8", "10", "12"]} /></Col>
            <Col><Label>Campaign Transition</Label><DefaultSelect value={d.transition} onChange={(v) => set({ transition: v })} fallback={DEFAULTS.transition} options={["None", "Fade", "Slide"]} /></Col>
          </Row>
          {d.maxRotation && d.maxRotation !== "Unlimited" && (
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, overflow: "hidden" }}>
                ...
                {d.slots.map((sl, i) => {
                  const o = SLOT_OWNERS[sl.owner];
                  const setSlot = (patch) => set({ slots: d.slots.map((q, k) => (k === i ? { ...q, ...patch } : q)) });
                  const pid = sl.partnerId || ANY_PARTNER;
          ...
          {caps(d).rotation ? (
            <Row>
              <Col><Label>Campaign Auto-Rotation</Label><DefaultSelect value={d.autoRotation} onChange={(v) => set({ autoRotation: v })} fallback={DEFAULTS.autoRotation} options={["Auto-Rotate On", "Auto-Rotate Off"]} /></Col>
              <Col><Label>Campaign Auto-Play</Label><DefaultSelect value={d.autoPlay} onChange={(v) => set({ autoPlay: v })} fallback={DEFAULTS.autoPlay} options={["Auto-Play On", "Auto-Play Off"]} /></Col>
            </Row>
          ) : (
            <Note>
              {caps(d).grid
        ...
        <Panel title="PHANTOM ZONE" open={open.phantom} onToggle={() => toggle("phantom")}
          badge={d.phantomEnabled ? <Pill color={T.success} bg="rgba(82,196,26,0.12)">defined</Pill> : <Pill color={T.muted} bg="rgba(0,0,0,0.04)">not defined</Pill>}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <span style={{ fontSize: 13.5 }}>Define phantom zone</span>
            <Toggle on={d.phantomEnabled} onChange={(v) => set({ phantomEnabled: v, features: v ? d.features : { ...d.features, qr_control: { ...d.features.qr_control, on: false } } })} />
          </div>
          {d.phantomEnabled ? (
            <>
              <Row>
                <Col><Label info>Phantom Area(s) Size</Label>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <input type="number" value={d.phantomW} onChange={(e) => set({ phantomW: Number(e.target.value) })} style={{ ...inputStyle, width: 62 }} />
                    <span style={{ color: T.muted, fontSize: 13 }}>H</span>
                    <input type="number" value={d.phantomH} onChange={(e) => set({ phantomH: Number(e.target.value) })} style={{ ...inputStyle, width: 62 }} />
                  </div></Col>
                <Col><Label>Phantom Area(s) Position</Label>
                  <DefaultSelect value={d.phantomPosition} onChange={(v) => set({ phantomPosition: v })} fallback={DEFAULTS.phantomPosition} options={["Top Left", "Top Right", "Bottom Left", "Bottom Right", "Center"]} /></Col>
              </Row>
              <Row>
                <Col><Label>Phantom Area(s) Sizing Mode</Label>
                  <select value={d.phantomSizing} onChange={(e) => set({ phantomSizing: e.target.value })} style={inputStyle}>
        ...
        <Panel title="ENABLED FEATURES" open={open.features} onToggle={() => toggle("features")}>
          ...
          {FEATURES.filter((f) => (FEATURE_TOUCH_POINTS[f.key] || ALL_TP).includes(d.touchPoint)).map((f) => {
            ...
            const cfg = d.features[f.key] || { on: false };
            const setCfg = (patch) => set({ features: { ...d.features, [f.key]: { ...cfg, ...patch } } });
            return (
        ...
        <Panel title="MULTI-ZONE LAYOUT" open={open.zones} onToggle={() => toggle("zones")}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            ...
            <Toggle on={d.zonesEnabled} onChange={(v) => set({ zonesEnabled: v, zones: v && d.zones.length === 0 ? [{ id: "z1", name: "Zone 1", x: 0, y: 0, w: 50, h: 100, playlist: ensureZonePlaylist(1) }, { id: "z2", name: "Zone 2", x: 50, y: 0, w: 50, h: 100, playlist: ensureZonePlaylist(2) }] : d.zones })} />
          </div>
          {!d.zonesEnabled ? (
            ...
            <>
              <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
                ...
                {[2, 3, 4, 6].map((n) => (
                  <Btn key={n} style={{ height: 26, fontSize: 12, padding: "0 10px" }}
                    onClick={() => set({ zones: Array.from({ length: n }, (_, i) => ({ id: `z${i + 1}`, name: `Zone ${i + 1}`, x: +(i * (100 / n)).toFixed(1), y: 0, w: +(100 / n).toFixed(1), h: 100, playlist: ensureZonePlaylist(i + 1) })) })}>
                    {n}
                  </Btn>
                ))}
              </div>
              {d.zones.map((z, i) => (
                <div key={z.id} style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: 6, padding: 10, marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    <input value={z.name} onChange={(e) => set({ zones: d.zones.map((q, k) => (k === i ? { ...q, name: e.target.value } : q)) })}
                      style={{ ...inputStyle, height: 28, fontSize: 13, flex: 1 }} />
                    {d.zones.length > 1 && <Icon name="close" size={16} style={{ color: T.muted, cursor: "pointer" }} onClick={() => set({ zones: d.zones.filter((_, k) => k !== i) })} />}
                  </div>
                  <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
                    {[["x", "X %"], ["y", "Y %"]].map(([k, l]) => (
                      <div key={k} style={{ flex: 1 }}>
                        <div style={{ fontSize: 11, color: T.muted, marginBottom: 2 }}>{l}</div>
                        <input type="number" value={z[k]} onChange={(e) => set({ zones: d.zones.map((q, j) => (j === i ? { ...q, [k]: Number(e.target.value) } : q)) })}
                          style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
                      </div>
                    ))}
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, color: T.muted, marginBottom: 2 }}>Width px</div>
                      <input type="number" value={Math.round((z.w / 100) * d.w)}
                        onChange={(e) => set({ zones: d.zones.map((q, j) => (j === i ? { ...q, w: (Number(e.target.value) / d.w) * 100 } : q)) })}
                        style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, color: T.muted, marginBottom: 2 }}>Height px</div>
                      <input type="number" value={Math.round((z.h / 100) * d.h)}
                        onChange={(e) => set({ zones: d.zones.map((q, j) => (j === i ? { ...q, h: (Number(e.target.value) / d.h) * 100 } : q)) })}
                        style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
                    </div>
                  </div>
                  <div style={{ fontSize: 11, color: T.muted, marginBottom: 3 }}>Playlist</div>
                  <select value={z.playlist} onChange={(e) => set({ zones: d.zones.map((q, k) => (k === i ? { ...q, playlist: e.target.value } : q)) })}
                    style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
              ...
              <Btn variant="outline" style={{ height: 28, fontSize: 12.5 }}
                onClick={() => set({ zones: [...d.zones, { id: `z${Date.now()}`, name: `Zone ${d.zones.length + 1}`, x: 0, y: 0, w: 25, h: 100, playlist: ensureZonePlaylist(d.zones.length + 1) }] })}>
...
function WebPreview({ d, plName, set, partners, companyLists }) {
  ...
  const el = webEl(d.elementType);
  const c = caps(d);
  const B0 = BREAKPOINTS.find((x) => x.key === bp);
  const vw = d[B0.wKey] ?? B0.defW;
  const frameW = Math.max(140, Math.round(280 * (vw / 1200)));
  const cols = c.grid ? { desktop: d.cols, tablet: d.colsTablet, mobile: d.colsMobile }[bp] : 1;
  const total = d.maxRotation && d.maxRotation !== "Unlimited" ? Number(d.maxRotation) : (c.grid ? 6 : 3);
  const shownAt = { desktop: d.items, tablet: d.itemsTablet, mobile: d.itemsMobile }[bp] ?? total;
  const slotCount = Math.min(total, shownAt || total);
  const aspectPad = { "16:9": 56, "4:3": 75, "1:1": 100, "3:4": 133, auto: 70 }[d.itemAspect] || 75;
  const inset = d.widthMode === "contained" ? 10 : 0;
  const qrCfg = d.features.qr_control || {};
  ...
  const body = () => {
    ...
    if (c.grid) {
      ...
      const cellH = Math.max(26, Math.round(((frameW - (cols - 1) * ((d.gap || 0) / 3)) / cols) * (aspectPad / 100)));
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: d.gap / 3 }}>
          {Array.from({ length: rows }, (_, r) => (
            <div key={r} style={{ display: "flex", gap: d.gap / 3 }}>
              {Array.from({ length: cols }, (_, k) => {
                const i = r * cols + k;
                return i < slotCount ? <Cell key={k} i={i} h={cellH} /> : <div key={k} style={{ flex: 1 }} />;
              })}
            </div>
          ))}
        </div>
    ...
    const inView = c.carousel ? (d[B0.colKey] || 1) : 1;
    const peek = c.carousel ? (d[B0.peekKey] ?? 0) : 0;
    const gapPx = (d.gap || 0) / 3;
  ...
  return (
    <div>
      ...
      <div style={{ width: frameW, border: `1px solid ${T.border}`, borderRadius: 8, overflow: "hidden", background: "#fff", transition: "width .15s" }}>
        ...
        <div style={{ position: "relative", padding: `10px ${inset + 8}px`, background: d.background === "#000000" ? "#fff" : d.background }}>
          {body()}
          {qrCfg.on && (() => {
      ...
      {set && (() => {
        const B = BREAKPOINTS.find((x) => x.key === bp);
        return (
          <div style={{ marginTop: 12, border: `1px solid ${T.borderSubtle}`, borderRadius: 8, padding: 12, maxWidth: 300 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <input type="number" value={d[B.wKey] ?? B.defW} onChange={(e) => set({ [B.wKey]: Number(e.target.value) })}
                  style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
                <span style={{ fontSize: 11.5, color: T.muted }}>H</span>
                <input type="number" value={d[B.hKey] ?? B.defH} onChange={(e) => set({ [B.hKey]: Number(e.target.value) })}
                  style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
              </div>
            ...
            {c.carousel && (
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <select value={d[B.colKey]} onChange={(e) => set({ [B.colKey]: Number(e.target.value) })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
                    {[1,2,3,4].map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11.5, color: T.muted, marginBottom: 4 }}>Peek %</div>
                  <select value={d[B.peekKey] ?? 0} onChange={(e) => set({ [B.peekKey]: Number(e.target.value) })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
            ...
            {c.grid && (
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <select value={d[B.itemKey]} onChange={(e) => set({ [B.itemKey]: Number(e.target.value) })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                <select value={d.widthMode} onChange={(e) => set({ widthMode: e.target.value })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <select value={d.itemAspect} onChange={(e) => set({ itemAspect: e.target.value })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }}>
                  {["16:9","4:3","1:1","3:4","auto"].map((a) => <option key={a}>{a}</option>)}
                </select>
              </div>
              <div style={{ width: 66 }}>
                <div style={{ fontSize: 11.5, color: T.muted, marginBottom: 4 }}>Gap</div>
                <input type="number" value={d.gap} onChange={(e) => set({ gap: Number(e.target.value) })} style={{ ...inputStyle, height: 28, fontSize: 12.5 }} />
              </div>
            </div>
          </div>
        );
      ...
      <div style={{ marginTop: 10, fontSize: 11.5, color: T.muted, lineHeight: 1.5, maxWidth: 300 }}>
        <b>{el.name}</b> · {d.widthMode === "full" ? "full bleed" : `contained ${d.maxWidth}px`}
        {c.grid && ` · ${cols} col${cols > 1 ? "s" : ""} at ${bp}`}
        <div style={{ marginTop: 4, color: T.primary }}>
          {c.campaigns
            ? (c.grid ? `${slotCount} of ${total} item${total > 1 ? "s" : ""} rendered at ${bp}` : "1 item rendered")
            : "Renders no campaigns"}
        </div>
      </div>
    </div>
...
function Preview({ d, plName, set, partners, companyLists }) {
  ...
  const width = fitW;
  const scale = (d.w / width).toFixed(0);

  const pos = d.phantomPosition || DEFAULTS.phantomPosition;
  const qr = d.features.qr_control;
  const showQR = d.phantomEnabled && qr.on;
  const pw = (d.phantomW / d.w) * 100, ph = (d.phantomH / d.h) * 100;
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: boxW }}>
        <div style={{ width, height, background: d.background, borderRadius: 6, position: "relative", overflow: "hidden", border: `1px solid ${T.border}`, boxSizing: "border-box", flexShrink: 0 }}>
          {d.zonesEnabled ? d.zones.map((z, i) => (
            <div key={z.id} style={{ position: "absolute", left: `${z.x}%`, top: `${z.y}%`, width: `${z.w}%`, height: `${z.h}%`, border: `2px dashed ${ZONE_COLOURS[i % 6]}`, background: `${ZONE_COLOURS[i % 6]}1a`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 3, boxSizing: "border-box", overflow: "hidden" }}>
            ...
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 8.5, marginTop: 2, textAlign: "center", lineHeight: 1.3 }}>
              {Math.round((z.w / 100) * d.w)}×{Math.round((z.h / 100) * d.h)}px
            </div>
            <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 8, marginTop: 1, textAlign: "center", lineHeight: 1.25 }}>{plName(z.playlist)}</div>
            </div>
          )) : (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ color: "rgba(255,255,255,0.85)", fontSize: 13, fontWeight: 500 }}>{plName(d.defaultPlaylist)}</div>
          ...
          {d.phantomEnabled && (
            <div style={{ position: "absolute", ...place, width: `${pw}%`, height: `${ph}%`, minWidth: 22, minHeight: 22, background: showQR ? "#fff" : "rgba(255,255,255,0.14)", border: showQR ? "none" : "1px dashed rgba(255,255,255,0.5)", borderRadius: 4, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", boxShadow: showQR ? "0 2px 8px rgba(0,0,0,0.35)" : "none" }}>
              {showQR ? (
                sigPaired ? (
                  <>
                    <Icon name="smartphone" size={Math.max(13, Math.min(28, (qr.size / d.w) * width * 1.2))} style={{ color: qr.connectedColour }} />
                    <div style={{ width: 5, height: 5, borderRadius: 9999, background: T.success, marginTop: 2 }} />
                  </>
                ) : (
                  <Icon name="qr_code_2" size={Math.max(14, Math.min(34, (qr.size / d.w) * width * 1.6))} style={{ color: qr.colour }} />
                )
              ) : (
      ...
      <div style={{ marginTop: 12, fontSize: 12, color: T.muted, maxWidth: boxW, lineHeight: 1.6 }}>
        {d.w} × {d.h} · {d.zonesEnabled ? `${d.zones.length} zones` : "single zone"} ·{" "}
        {d.phantomEnabled ? (showQR ? "phantom zone with QR control" : "phantom zone defined, QR control off") : "no phantom zone"}
      </div>
        ...
        <div style={{ marginTop: 14, maxWidth: boxW }}>
          ...
          {d.zones.map((z, i) => (
            <div key={z.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: `1px solid ${T.borderSubtle}`, fontSize: 12.5 }}>
              ...
              <span style={{ color: T.muted, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{plName(z.playlist)}</span>
              <span style={{ color: T.micro, fontSize: 11.5 }}>{Math.round((z.w / 100) * d.w)}px</span>
            </div>
          ))}
        </div>
      )}
...
function PlaylistManagement({ playlists, setPlaylists, types, goToType }) {
  ...
  const create = (n) => {
    if (!n || !n.trim()) return;
    setPlaylists([...playlists, { id: `pl_${Date.now()}`, name: n.trim(), autoFor: null }]);
  };
                    ...
                    <>
                      ...
                      {p.autoFor && (
                        <div style={{ marginTop: 3 }}>
                          <Pill color={T.muted} bg="rgba(0,0,0,0.04)">auto-created with {types.find((t) => t.id === p.autoFor)?.name || p.autoFor}</Pill>
                        </div>
                      )}
                    </>
                  )}
...
function LayoutComposer({ types, templates, setTemplates, playlists }) {
  ...
  const palette = types.filter((t) => isWebTP(t.touchPoint) && t.elementType !== "qr_control");
  const qrElements = types.filter((t) => t.elementType === "qr_control");
        ...
        {isMobile
          ...
          : palette.map((t) => (
              <div key={t.id} draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", `new:${t.id}`)}
                ...
                <Icon name={webEl(t.elementType).icon} size={15} style={{ color: T.primary }} />
                <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</span>
              </div>
            ))}
      </div>
...
function TemplatePreview({ tpl, types, bp, kind, paired }) {
  ...
  const idle = qrEl?.qrIdle || { qrColour: "#000000", qrSize: 120, showPoweredBy: true, poweredByText: "Powered by Personalisation Hub" };
  const conn = qrEl?.qrConnected || { icon: "smartphone", iconColour: "#169bc2", showPoweredBy: true };
    ...
    <div style={{ width: w, border: `1px solid ${T.border}`, borderRadius: kind.frame === "phone" ? 16 : 8, overflow: "hidden", background: "#fff", position: "relative", transition: "width .15s" }}>
      ...
      <div style={{ maxHeight: 380, overflowY: "auto", background: tpl.background, padding: tpl.widthMode === "contained" ? "8px 10px" : "8px 0" }}>
        {tpl.rows.map((row, i) => {
          ...
          const cols = el.plays === "simultaneous" ? { desktop: t.cols, tablet: t.colsTablet, mobile: t.colsMobile }[bp] || 1 : 1;
          const shown = el.plays === "simultaneous" ? ({ desktop: t.items, tablet: t.itemsTablet, mobile: t.itemsMobile }[bp] || mock.length) : 1;
