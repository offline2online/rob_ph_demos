...
**The PH Agent Console MCP connector can do the same, one document at a
time** — `set_project_requirements`, `set_project_readme`,
`set_project_artifact` and `update_interface` (see "Who can use this, and
their AI agents (MCP)" below). A connected agent should use those directly;
this script is for a person with `BOARD_API_KEY` pushing a whole folder up in
one go, or for a session with no MCP connection.
