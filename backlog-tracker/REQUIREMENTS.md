...
created, and left unset only for a project that genuinely owns no single
folder (this one owns both `backlog-tracker/` and `faq/`). It is not in the
MCP server's `PROJECT_WRITABLE_FIELDS`, so an agent cannot set it; the
console or a runner with the board credential does. The New Project modal
does not yet ask for it — see README, "Adding a project".
